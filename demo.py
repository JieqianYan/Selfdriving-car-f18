...
for k, b in enumerate(bbox):
    ...
    for e in edges.T:
        ...
        if(count >= 3):
            ...
            b_2 = min(b_2, min(vert_2D[1, e[1]], vert_2D[1, e[1]]))
            b_3 = max(b_3, max(vert_2D[1, e[1]], vert_2D[1, e[1]]))
            #print([b_0, b_0], [b_0, b_3])
            print(vert_2D[0, e])
            print(vert_2D[1, e])
            print(e)
            #ax1.plot(vert_2D[0, e], vert_2D[1, e], color=clr)
        ax2.plot(vert_3D[0, e], vert_3D[1, e], vert_3D[2, e], color=clr)
    ax1.plot([b_0, b_0], [b_2, b_3], color=clr)
    ax1.plot([b_1, b_1], [b_2, b_3], color=clr)
    ax1.plot([b_0, b_1], [b_2, b_2], color=clr)
    ax1.plot([b_0, b_1], [b_3, b_3], color=clr)
